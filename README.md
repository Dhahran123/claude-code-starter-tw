...
| 兩家都沒有、只想先試 | 兩者皆可，都有不用打指令的桌面版。本包的設定腳本以 Claude Code 為基準撰寫，Codex 靠對照表執行 |
...
3. **接上外部工具**：操作瀏覽器、讀網頁
4. **裝你要的技能模組**：文書處理（Word／PPT／Excel／PDF）是常用的起手組合；
   醫療文獻、網頁簡報、社群文案——依你的工作挑，每一項都可以不裝
...
*有問題直接問你的 AI，它比任何說明書都有耐心；文件有錯歡迎到 repo 開 issue。*
