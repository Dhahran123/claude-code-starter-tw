...
你的任務：照本文件的階段順序，帶他完成個人化設定——
**先透過訪談認識他，再依他的工作裝對的模組**，不是把所有東西全裝。

**執行者可能是 Claude Code，也可能是 Codex。** 本文件以 Claude Code 寫成；
你若是 Codex，先讀同 repo 的 `DUAL.md` 第一節「Codex 對照表」，把本文件裡
Claude 專屬的指令與路徑（`claude mcp add`、`~/.claude/…`、桌面版做法、Connectors）
換成對照表的做法再執行；對照表標「跳過」的階段就跳過。
...
4. 指令失敗最多重試兩次，之後分兩種處理：
   - **硬前置**（本機 session、Node.js／npx、git、寫入 `~/.claude.json`、階段 2 選裝的基礎 MCP）：
...
若有放「資料紅線」這節，用白話講一次道理（README「一件值得知道的事」那段），
並補一句安心話：這是 Claude 幫他把關，不是要他用得提心吊膽。
...
白話：MCP 像「幫助理接上各種工具的插座」。下面兩個是多數用途都會用到的基礎工具，
介紹後由使用者決定要不要裝；其他的（例如醫學文獻資料庫）放在階段 3 的模組裡，選了才裝。
...
本文件列的來源皆經過安全稽核，但這個習慣值得保持。

### 3a. 基礎技能（常用起手組合——介紹後由使用者決定，可整組跳過）
...
Plus 以上較不會中斷；沒有帳號或不想另付就跳過本階段，說明以後想裝隨時說一聲）。有的話另外告訴他：想更進一步——兩邊共用員工手冊與技能、
...
⚠️ 界線（講給使用者聽）：**任何 `/codex:` 開頭的指令**，都會把相關內容
（對話、檔案、程式碼）送到 OpenAI 的雲端——不是只有 transfer／delegate 才算。
若使用者的 `~/.claude/CLAUDE.md` 有「資料紅線」節，替他補一行：
**含病患／客戶敏感資料的工作一律不使用 `/codex:` 指令。**

---

*維護註記（給維護者）：本文件內的安裝指令與 repo 路徑會隨時間過期；
發現壞掉歡迎開 issue，或直接改。*
